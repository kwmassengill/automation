    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if not OAUTH_CREDENTIALS_FILE.exists():
                logger.error(f"OAuth credentials file not found at {OAUTH_CREDENTIALS_FILE}")
                raise FileNotFoundError(f"OAuth credentials file not found: {OAUTH_CREDENTIALS_FILE}")
