...
def get_gmail_service():
    ...
    if os.path.exists(OAUTH_TOKEN_PATH):
        creds = Credentials.from_authorized_user_file(OAUTH_TOKEN_PATH, GMAIL_SCOPES)
    ...
    if not creds or not creds.valid:
        if not os.path.exists(OAUTH_CREDENTIALS_PATH):
            logger.error(f"OAuth credentials file not found at {OAUTH_CREDENTIALS_PATH}")
            logger.error("Please set up Gmail OAuth credentials first.")
            return None
            
        flow = InstalledAppFlow.from_client_secrets_file(
            OAUTH_CREDENTIALS_PATH, GMAIL_SCOPES)
        creds = flow.run_local_server(port=0)
        
        # Save token for future use
        with open(OAUTH_TOKEN_PATH, 'w') as token:
            token.write(creds.to_json())
        logger.info("Gmail OAuth token saved for future use")
